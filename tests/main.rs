...
fn test_empty_input_text() {
    ...
    let bitmaps: Vec<_> = ""
        .chars()
        .filter_map(|c| bdf.get_bitmap(c as u32))
        .collect();
    assert!(
        bitmaps.is_empty(),
        "Bitmaps should be empty for empty input text"
    );
}
...
fn test_default_pixel_and_space() {
    ...
    assert!(
        ...
        "Rendered text should contain the default pixel character"
    );
    assert!(
        rendered_text.contains("　"),
        "Rendered text should contain the default space character"
    );
}
...
fn test_custom_pixel_and_space() {
    ...
    assert!(
        rendered_text.contains("#"),
        "Rendered text should contain the custom pixel character"
    );
    assert!(
        rendered_text.contains("."),
        "Rendered text should contain the custom space character"
    );
}
...
fn test_unsupported_character_warning() {
    ...
    assert!(
        bitmap.is_none(),
        "Bitmap should be None for unsupported characters"
    );
}
...
fn test_multiple_characters_rendering() {
    ...
    assert_eq!(
        bitmaps.len(),
        3,
        "Bitmaps should contain three items for 'ABC'"
    );
